...
for line in testData:
	...
	winningIndexVocab = vocab_probs.index(max(vocab_probs))
	winningCategoryVocab = naive_bayes.categories[winningIndexVocab]

	winningIndexPos = pos_probs.index(max(pos_probs))
	winningCategoryPos = pos_classifier.categories[winningIndexPos]
	print actualCategory + "\t" + winningCategoryVocab + "\t" + winningCategoryPos

	#Evaluate accuracy
	if winningCategoryVocab == actualCategory:
		numCorrect += 1
	else:
		numIncorrect += 1
